...
| `discovery-subaccount/` | A second real discovery run, this time for a **mutating, irreversible** capability (`member.subaccount.open`) — the first evidence anywhere in this repo of the `requires_approval` gate firing for real, on both the discovery and replay side, plus two real bugs found and fixed while building it. See its own README. |
...
| `escalation/` | A real intervention during **replay**: raised, handed off, resumed on the same live session, re-anchored, and completed. See its own README for the full cycle and what's mocked. |
| `discovery-escalation/` | The same mechanism, triggered during **discovery** instead: the agent hits a dialog outside its closed action vocabulary, a human clears it on the same live session, control returns to the model. See its own README. |
